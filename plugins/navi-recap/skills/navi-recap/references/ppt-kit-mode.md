...
navi-recap can build a deck two ways — **both build on ppt-kit, so both support
...
> navi-recap does **not** use presenter mode. It is intentionally left out of
...
but navi-recap does not use or document it — ignore it.)
...
|---|---|
| **Default — keeps the navi-recap identity** | `navi-signal` (signal-green on near-black, serif headlines, mono labels) |
...
`navi-signal` lives in `themes/` (navi-recap-owned); the other 36 live in
