...
> reveals, progress bar, theme cycling). It is **generated from upstream** by
> `scripts/build-single-template.sh` (run via `update-ppt.sh`) — never hand-edit
> it. Your job is to fill in slides and pick a theme; the sections below explain
